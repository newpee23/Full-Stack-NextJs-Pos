...
export const handleAddPosition = async (body: dataVerifyPosition, res: NextApiResponse) => {
    ...
    if (verifyPosition.length > 0) {
        return res.status(404).json({ message: verifyPosition, position: null, status: false });
    }
    ...
    if (checkCompanyName.length > 0) {
        return res.status(404).json({ message: checkCompanyName, position: null, status: false });
    }
    // addPosition
    const addPosition = await insertPosition(body);
    if (!addPosition) {
        return res.status(404).json({ message: "An error occurred saving data.", position: null, status: false });
    }
...
export const handleUpdatePosition = async (body: dataVerifyPosition, res: NextApiResponse) => {
    ...
    if(!body.id || isNaN(Number(body.id))){
        return res.status(404).json({ message: "Please specify positionId.", position: null, status: false });
    }
    // VerifyPositionData
    const verifyPosition = verifyPositionBody(body);
    if (verifyPosition.length > 0) {
    ...
    if(!checkPositionId || (Array.isArray(checkPositionId) && checkPositionId.length === 0)){
        return res.status(404).json({ message: `No Position information found from Id. ${body.id}`, position: null, status: false })
    }
    // ตรวจว่ามีข้อมูล companyId และ name ในระบบมั้ย
