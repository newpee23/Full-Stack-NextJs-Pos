...
export const handleGetBranchById = async (res: NextApiResponse, id: number) => {
    ...
    if (!branch) {
        return res.status(404).json({ message: `No branch found with Id : ${id}`, branch: null, status: false });
    }
...
export const handleGetBranchByCompanyId = async (res: NextApiResponse, companyId: number) => {
    ...
    if (!branch || branch?.length === 0) {
        return res.status(404).json({ message: `No branch found with companyId : ${companyId}`, branch: null, status: false });
    }
...
export const handleGetAllBranch = async (res: NextApiResponse) => {
    ...
    if (branch === null || branch.length === 0) {
        res.status(404).json({ message: `No branchs found`, branch: null, status: false });
        return;
    }
...
export const handleDeleteBarnch = async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    if (!query.id) {
        return res.status(404).json({ message: "Please specify Id", branch: null, status: false });
    }
    ...
    const branch = await getBranchById(id);
    if (!branch) {
        return res.status(404).json({ message: `No branch information found from Id : ${id}`, branch: null, status: false });
    }
    // deleteBranch
    const deleteBranch = await deleteDataBranch(id);
    if (!deleteBranch) {
        return res.status(404).json({ message: "An error occurred deleting data.", branch: null, status: false });
    }
...
export const handleAddBranch = async (body: dataVerifyBranch, res: NextApiResponse) => {
    ...
    if (verifyCompany.length > 0) {
        return res.status(404).json({ message: verifyCompany, branch: null, status: false });
    }
    ...
    if (checkCompanyName.length > 0) {
        return res.status(404).json({ message: checkCompanyName, branch: null, status: false });
    }
    // addBranch
    const addBranch = await insertBranch(body);
    if (!addBranch) {
        return res.status(404).json({ message: "An error occurred saving data.", branch: null, status: false });
    }
...
export const handleUpdateBranch = async (body: dataVerifyBranch, res: NextApiResponse) => {
    ...
    if (verifyBranch.length > 0) {
        return res.status(404).json({ message: verifyBranch, branch: null, status: false });
    }
    // ตรวจว่าใน DB มี ข้อมูล id หรือยัง
    const checkBranchId = await getBranchById(body.id);
    if (!checkBranchId) {
        return res.status(404).json({ message: `No branch information found from Id. ${body.id}`, branch: null, status: false })
    }
    ...
    if (checkBranchName) {
        return res.status(404).json({ message: `Found company information : ${body.name} has already been used in the system`, branch: null, status: false });
    }
    // update Branch
    const updateBranch = await updateDataBranch(body.id, body);
    if (!updateBranch) {
        return res.status(404).json({ message: "An error occurred editing data.", branch: null, status: false });
    }
