...
const addDataTransaction = async (token: string | undefined, transactionData: dataVerifyTransaction, setLoadingQuery: React.Dispatch<React.SetStateAction<number>>): Promise<addTransactionType | null> => {
  try {
...
export const useAddDataTransaction = () => {
  return useMutation(
      ...
      }) => addDataTransaction(variables.token, variables.transactionData, variables.setLoadingQuery)
  );
};
