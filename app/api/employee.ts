...
const deleteEmployee = async (token: string | undefined, itemId: string): Promise<fetchEmployee | null> => {
  try {
    const response = await axios.delete(
      `${process.env.NEXT_PUBLIC_BASE_URL}/employee?id=${itemId}`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
...
const updateEmployee = async (token: string | undefined,employeeData: dataVerifyEmployee,setLoadingQuery: React.Dispatch<React.SetStateAction<number>>): Promise<addEmployeeType | null> => {
  try {
...
export const useUpdateDataEmployee = () => {
  return useMutation(
    (variables: {
      token: string | undefined;
      employeeData: dataVerifyEmployee;
      setLoadingQuery: React.Dispatch<React.SetStateAction<number>>;
    }) => updateEmployee(variables.token, variables.employeeData, variables.setLoadingQuery)
  );
};

export const useDeleteDataEmployee = () => {
  return useMutation((variables: { token: string | undefined; id: string }) => deleteEmployee(variables.token, variables.id));
};

export const useAddDataEmployee = () => {
  return useMutation(
    (variables: {
      token: string | undefined;
      employeeData: dataVerifyEmployee;
