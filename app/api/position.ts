import { fetchPosition, optionSelect, optionSelectCompany } from "@/types/fetchData";
...
const fetchOptions = async (token: string | undefined, companyId: number | undefined): Promise<optionSelectCompany[]> => {
  try {
    ...
    const optionBranch: optionSelectCompany[] = response.data.optionBranch;
    return optionBranch;
  } catch (error: unknown) {
    if (axios.AxiosError) {
      // The error is an instance of AxiosError
...
export const useSelectOpPosition = (token: string | undefined, companyId: number | undefined) => {
  return useQuery('selectOpAddPosition', () => fetchOptions(token, companyId), {
    refetchOnWindowFocus: false,
  });
};